...
def success_response(data: Optional[T] = None, message: Optional[str] = None, status_code: int = 200) -> JSONResponse:
    """Create and return a JSONResponse for a successful result."""

    return to_http_response(success(data=data, message=message), status_code=status_code)


def error_response(message: str, data: Optional[T] = None, status_code: int = 400) -> JSONResponse:
